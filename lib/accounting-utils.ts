import { supabase, safeNumber } from "./supabase"
import type { Account, AccountType, JournalEntry, JournalEntryLine } from "./supabase"
...
export class AccountingService {
  ...
  static async getDashboardStats(): Promise<DashboardStats> {
    try {
      ...
      // Use getAllAccountBalances to batch-load all balances in 2 queries instead of N+1
      const [balances, accounts, journalCount, accountCount] = await Promise.all([
        this.getAllAccountBalances(currentDate),
        supabase
          .from("accounts")
          .select(`id, account_types!inner(name)`)
          .eq("is_active", true),
        supabase
      ...
      if (accounts.data) {
        for (const account of accounts.data) {
          const typeName = (account as any).account_types?.name || ""
      ...
      return {
        totalAssets,  // Show real values, don't hide negatives
        netIncome,
        journalEntriesCount: journalCount.count || 0,
        activeAccountsCount: accountCount.count || 0,
      }
    } catch (error) {
  ...
  static async getChartOfAccounts(): Promise<Account[]> {
    try {
      ...
      const { data: accountsData, error: accountsError } = await supabase
        ...
        // Include inactive accounts too — they may still have transactions.
        // Excluding them causes the trial balance to appear unbalanced.
        .order("code")
        .limit(2000)
  ...
  static async createJournalEntry(entry: {
    ...
    try {
      ...
      // Validate double-entry using safeNumber for consistency with storage
      const totalDebits = Math.round(entry.lines.reduce((sum, line) => sum + safeNumber(line.debit_amount), 0) * 100) / 100
      const totalCredits = Math.round(entry.lines.reduce((sum, line) => sum + safeNumber(line.credit_amount), 0) * 100) / 100
      ...
      const lines = entry.lines.map((line, index) => {
        ...
        return {
          ...
          debit_amount: safeNumber(line.debit_amount),
          credit_amount: safeNumber(line.credit_amount),
          line_number: index + 1,
  ...
  static async getJournalEntryById(id: string): Promise<any | null> {
    try {
      ...
      // Batch-fetch all account details for all lines in ONE query (fixes N+1)
      const linesWithAccounts = lines || []
      if (linesWithAccounts.length > 0) {
        const uniqueAccountIds = [...new Set(linesWithAccounts.map((l: any) => l.account_id).filter(Boolean))]
        ...
        if (uniqueAccountIds.length > 0) {
          const { data: accountsData } = await supabase
            ...
            .select(`
              id, code, name,
              account_types (id, name, normal_balance, description)
            `)
          ...
          for (const line of linesWithAccounts) {
            (line as any).accounts = accountMap.get(line.account_id) || null
          }
        }
      }
  ...
  static async updateJournalEntry(entryId: string, data: {
    ...
    try {
      ...
      const totalDebit = Math.round(data.lines.reduce((sum, line) => sum + safeNumber(line.debit_amount), 0) * 100) / 100
      const totalCredit = Math.round(data.lines.reduce((sum, line) => sum + safeNumber(line.credit_amount), 0) * 100) / 100
      const isBalanced = Math.abs(totalDebit - totalCredit) < 0.01

      // Validate that entry is balanced before updating
      if (!isBalanced) {
        throw new Error(`Journal entry is not balanced. Total debits (${totalDebit}) must equal total credits (${totalCredit})`)
      }

      // Step 1: Insert new lines FIRST (safer - if this fails, old lines are untouched)
      ...
      // Step 4: Update the header with correct totals
  ...
  // Repair is_balanced flags: recompute from actual line data and fix any wrong flags in DB
  static async repairBalanceFlags(): Promise<{ fixed: number; checked: number; stillUnbalanced: string[] }> {
    try {
      // Fetch all entries
      let allEntries: any[] = []
      let offset = 0
      while (true) {
        const { data, error } = await supabase
          .from("journal_entries")
          .select("id, entry_number, is_balanced, total_debit, total_credit")
      ...
      for (const line of allLines) {
        ...
        if (!entryTotals.has(eid)) entryTotals.set(eid, { debit: 0, credit: 0 })
        const t = entryTotals.get(eid)!
        t.debit += safeNumber(line.debit_amount)
        t.credit += safeNumber(line.credit_amount)
      }

      let fixed = 0
      const stillUnbalanced: string[] = []

      for (const entry of allEntries) {
        const totals = entryTotals.get(entry.id)
        const actualTotalDebit  = safeNumber(totals?.debit  ?? 0)
        const actualTotalCredit = safeNumber(totals?.credit ?? 0)
        const actuallyBalanced  = Math.abs(actualTotalDebit - actualTotalCredit) < 0.01 && actualTotalDebit > 0

        if (!actuallyBalanced) {
          stillUnbalanced.push(entry.entry_number)
        }

        // ALWAYS sync total_debit, total_credit, and is_balanced from actual lines.
        // The stored header fields can drift when lines are added/removed outside the app.
        const headerDebitWrong   = Math.abs(safeNumber(entry.total_debit)  - actualTotalDebit)  > 0.01
        const headerCreditWrong  = Math.abs(safeNumber(entry.total_credit) - actualTotalCredit) > 0.01
        const headerFlagWrong    = entry.is_balanced !== actuallyBalanced

        if (headerDebitWrong || headerCreditWrong || headerFlagWrong) {
          const { error } = await supabase
            .from("journal_entries")
            .update({
              is_balanced:  actuallyBalanced,
      ...
      return { fixed, checked: allEntries.length, stillUnbalanced }
    } catch (error) {
      console.error("Error repairing balance flags:", error)
      throw new Error("Failed to repair balance flags")
    }
  }
  ...
  static async findUnbalancedEntries(startDate?: string, endDate?: string): Promise<Array<{
    ...
    try {
      ...
      throw new Error("Failed to find unbalanced entries")
    }
  }
  ...
  static async getTrialBalance(startDate?: string, endDate?: string): Promise<TrialBalanceItem[]> {
    try {
      ...
      //    the trial balance to appear unbalanced.
      ...
      for (const row of allLines) {
        ...
        if (!balanceMap.has(accountId)) {
          // Account referenced in a line but missing from chart of accounts
          balanceMap.set(accountId, { debit: 0, credit: 0, closing: 0 })
        }
        const cur = balanceMap.get(accountId)!
        cur.debit  += safeNumber(row.debit_amount)
        cur.credit += safeNumber(row.credit_amount)
      }

      // Round accumulated totals and compute closing balance for ALL entries in balanceMap
      for (const [, cur] of balanceMap) {
        cur.debit   = safeNumber(cur.debit)
        cur.credit  = safeNumber(cur.credit)
        cur.closing = safeNumber(cur.debit - cur.credit)
      }

      // Log any account IDs in lines that are missing from the chart of accounts
      const missingAccountIds: string[] = []
      for (const [aid] of balanceMap) {
        if (!accountIds.has(aid)) {
          const m = balanceMap.get(aid)!
          if (m.debit > 0 || m.credit > 0) {
            missingAccountIds.push(aid)
            console.warn(`Trial balance: account ${aid} has transactions (D:${m.debit} C:${m.credit}) but is NOT in chart of accounts`)
          }
        }
      }
      if (missingAccountIds.length > 0) {
        console.warn(`Trial balance: ${missingAccountIds.length} account(s) with transactions are missing from the chart of accounts. This causes the trial balance to appear unbalanced. Run "Repair Balance Flags" and check your chart of accounts.`)
      }

      // Build parent-child map with validation
      const childrenMap = new Map<string, string[]>()
      // Reuse accountIds set from above (already contains all account IDs)

      for (const account of accounts) {
        if (account.parent_account_id) {
          // Validate parent account exists
          if (!accountIds.has(account.parent_account_id)) {
            console.warn(`Account ${account.code} references non-existent parent, skipping hierarchy`)
            continue
          }
      ...
      // Calculate total balance including children (recursive with circular reference detection)
      const totalBalanceMap = new Map<string, number>()
      const visitedStack = new Set<string>()

      const calculateTotalBalance = (accountId: string, stack = new Set<string>()): number => {
        if (totalBalanceMap.has(accountId)) return totalBalanceMap.get(accountId)!

        // Detect circular references
        if (stack.has(accountId)) {
          console.error(`Circular reference detected in account hierarchy: ${accountId}`)
          return 0
        }
        ...
        for (const childId of children) {
          childrenTotal += calculateTotalBalance(childId, stack)
        }
      ...
      // Add rows for any accounts referenced in lines but missing from the chart.
      // Without this, those amounts are silently excluded from the grand totals,
      // making the trial balance appear unbalanced even when all entries are correct.
      for (const [aid, bal] of balanceMap) {
        if (accountIds.has(aid)) continue          // already included above
        if (bal.debit === 0 && bal.credit === 0) continue  // nothing to show
        trialBalanceItems.push({
          account_id: aid,
          account_code: "????",
          account_name: `[Deleted/Inactive Account: ${aid.substring(0, 8)}]`,
          ...
          closing_balance: bal.closing,
          total_balance: bal.closing,
          has_children: false,
        })
      }
    ...
    try {
      ...
      if (startDate && endDate) {
        ...
        if (jeError) {
          ...
          // If too many IDs for a single .in(), we need to handle it differently
          if (journalEntryIds.length <= 100) {
            query = query.in("journal_entry_id", journalEntryIds)
          } else {
            // For large ID sets, fetch all lines and filter in memory
            // Store the IDs for post-query filtering
            const journalEntryIdSet = new Set(journalEntryIds)
            const { data: allData, error: allError } = await query.limit(50000)
            if (allError) {
              console.error("Error loading general ledger:", allError)
              throw new Error(`Failed to load general ledger: ${allError.message || allError}`)
            }
            // Filter by journal entry IDs in memory
            const filteredData = (allData || []).filter((entry: any) => journalEntryIdSet.has(entry.journal_entry_id))
            // Build account map for enrichment
            const enrichedResult = filteredData.sort((a: any, b: any) => {
              ...
              return dateA !== dateB ? dateA - dateB : new Date(a.created_at || 0).getTime() - new Date(b.created_at || 0).getTime()
            }).map((entry: any) => {
              const account = accountMap.get(entry.account_id)
              return { ...entry, account_code: account?.code || '', account_name: account?.name || '', is_child_account: entry.account_id !== accountId }
            })
  ...
  static async getBalanceSheet(asOfDate: string): Promise<any> {
    try {
      ...
      // Add Net Income to Equity section (show actual value, positive = profit, negative = loss)
      if (netIncome !== 0) {
        equity.push({
          name: "Net Income",
          amount: netIncome,
  ...
  static async getIncomeStatement(startDate: string, endDate: string): Promise<any> {
    try {
      ...
      // BATCH approach: Fetch ALL relevant transactions in the date range at once (fixes N+1)
      // Step 1: Get all journal entries in the date range
      ...
      const validEntryIds = new Set((periodEntries || []).map(je => je.id))

      // Step 2: Get ALL transaction lines for ALL accounts at once
      const allAccountIds = accounts.map(a => a.id)
      let allTransactions: any[] = []

      // Fetch in chunks of 100 account IDs
      for (let i = 0; i < allAccountIds.length; i += 100) {
        const chunk = allAccountIds.slice(i, i + 100)
        const { data: txns } = await supabase
          .from("journal_entry_lines")
          .select("account_id, debit_amount, credit_amount, journal_entry_id")
          .in("account_id", chunk)
        if (txns) allTransactions = allTransactions.concat(txns)
      }

      // Filter to only transactions within the date range
      const periodTransactions = allTransactions.filter(t => validEntryIds.has(t.journal_entry_id))

      // Build a map of account_id -> { debitTotal, creditTotal }
      const activityMap = new Map<string, { debitTotal: number; creditTotal: number }>()
      for (const t of periodTransactions) {
        const existing = activityMap.get(t.account_id) || { debitTotal: 0, creditTotal: 0 }
        existing.debitTotal += safeNumber(t.debit_amount)
        existing.creditTotal += safeNumber(t.credit_amount)
        activityMap.set(t.account_id, existing)
      }

      // Helper to get activity for an account and all its descendants
      const getDescendantActivity = (accountId: string): { debitTotal: number; creditTotal: number } => {
        const ids = getAllDescendantIds(accountId)
        let debitTotal = 0
        let creditTotal = 0
        for (const id of ids) {
      ...
      for (const account of rootAccounts) {
        ...
        const activity = getDescendantActivity(account.id)
        ...
        // Show actual amount (positive for normal activity, negative for reversed)
        const accountData = {
          name: account.name,
  ...
  static async getCashFlowStatement(startDate: string, endDate: string): Promise<CashFlowStatement> {
    try {
      ...
      // Get ALL transactions for ALL accounts in chunked queries (Supabase .in() has URL length limits)
      const accountIds = accounts.map(acc => acc.id)
      let allTransactions: any[] = []

      // Chunk journalEntryIds into batches of 80 to avoid URL length limits
      const CHUNK_SIZE = 80
      for (let i = 0; i < journalEntryIds.length; i += CHUNK_SIZE) {
        const chunk = journalEntryIds.slice(i, i + CHUNK_SIZE)
        ...
        if (chunkError) {
          console.error("Error fetching transactions for cash flow (chunk):", chunkError)
          // Continue with other chunks instead of failing completely
          continue
        }
        if (chunkData) {
          allTransactions = allTransactions.concat(chunkData)
        }
      }
      ...
      for (const transaction of allTransactions || []) {
        ...
        totals.debitTotal += safeNumber(transaction.debit_amount)
        totals.creditTotal += safeNumber(transaction.credit_amount)
      }
  ...
  private static async getAccountBalance(accountId: string, asOfDate: string): Promise<number> {
    try {
      ...
      if (openingBalance && !openingError) {
        openingBal = safeNumber(openingBalance.balance)
      }
      ...
      for (const trans of transactions || []) {
        debitTotal += safeNumber(trans.debit_amount)
        creditTotal += safeNumber(trans.credit_amount)
      }

      // Handle account_types - may be object or array depending on query
      ...
      if (isDebitNormal) {
        return Math.round((openingBal + debitTotal - creditTotal) * 100) / 100
      } else {
        return Math.round((openingBal + creditTotal - debitTotal) * 100) / 100
      }
  ...
  static async getAllAccountBalances(asOfDate?: string): Promise<Map<string, { ownBalance: number; totalBalance: number }>> {
    try {
      ...
      // Create a map of account_id -> opening balance (using safeNumber for DECIMAL)
      const openingBalanceMap = new Map<string, number>()
      for (const ob of openingBalances || []) {
        openingBalanceMap.set(ob.account_id, safeNumber(ob.balance))
      }
      ...
      // Group transactions by account (using safeNumber for proper DECIMAL handling)
      ...
      for (const trans of allTransactions || []) {
        ...
        current.debits += safeNumber(trans.debit_amount)
        current.credits += safeNumber(trans.credit_amount)
  ...
  static async markSupplyDone(id: string): Promise<any> {
    try {
      ...
      const updateData: any = {
        supply_done: true,
        supply_done_at: new Date().toISOString(),
        supply_done_by: currentUser.id,
      }
